...
def fetch_data():
    flats_json = urlopen('https://www.sreality.cz/api/cs/v2/estates?category_main_cb=1&category_type_cb=1&per_page=20&tms=1663433765480')
    flats = json.loads(flats_json.read().decode("utf-8"))

    flats_db = []
    for i in range(20):
        flats_db.append((flats["_embedded"]["estates"][i]["name"], flats["_embedded"]["estates"][i]["_links"]["images"][0]["href"]))
    return flats_db
